...
export class Global {
    ...
    public static showFullEventDateTime: boolean = true;
    public static showFullEventTooltip: boolean = true;
...
export enum StorageFlag {
    ...
    SHOW_FULL_EVENT_DATETIME = 'ah-show-full-event-datetime',
    SHOW_FULL_EVENT_TOOLTIP = 'ah-show-full-event-tooltip',
...
export async function readStorageToGlobal(): Promise<void> {
    ...
    Global.showFullEventDateTime = await getStorage<boolean>(StorageFlag.SHOW_FULL_EVENT_DATETIME, true, { alsoInit: true });
    Global.showFullEventTooltip = await getStorage<boolean>(StorageFlag.SHOW_FULL_EVENT_TOOLTIP, true, { alsoInit: true });
